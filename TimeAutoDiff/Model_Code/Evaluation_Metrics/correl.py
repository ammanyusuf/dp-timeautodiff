...
#         # If the ratio is below the threshold, consider the column as categorical
#         if n_unique >= 1 and n_unique <= 25 or dataframe[col].dtype == 'int64':
#             categorical_columns.append(i)  # Use index
#         elif dataframe[col].dtype == 'object':
#             categorical_columns.append(i)  # Use index
#         else:
#             continuous_columns.append(i)  # Use index
...
def compute_correlation(df, continuous_columns, categorical_columns):
    ...
    num_mat = pd.DataFrame(df.loc[:, continuous_columns])  # Use .loc for column names
    cat_mat = pd.DataFrame(df.loc[:, categorical_columns])  # Use .loc for column names
