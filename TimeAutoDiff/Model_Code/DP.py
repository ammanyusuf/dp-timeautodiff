...
# Define the cyclical encoding function for seconds
def cyclical_encode_seconds(df):
    # Cyclic encoding based on seconds within each window
    seconds_in_window = 1  # Assuming 1-second windows
    df['sin_seconds'] = np.sin(2 * np.pi * df['second_within_window'] / seconds_in_window)
    df['cos_seconds'] = np.cos(2 * np.pi * df['second_within_window'] / seconds_in_window)
    return df
...
def partition_multi_seq_sec(real_df, threshold, column_to_partition):    
    ...
    # Partition the multi-sequence data's date information
    df2 = cyclical_encode_seconds(real_df); 
    partitioned_tensors_ts = torch.zeros(len(unique_values), int(len(processed_data[0,:,:])/len(unique_values)), 8)
    time_info = torch.tensor(df2.iloc[:,-8:].values).unsqueeze(0)
