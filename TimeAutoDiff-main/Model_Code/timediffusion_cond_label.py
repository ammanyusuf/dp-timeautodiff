...
class BiRNN_score(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, diffusion_steps, time_dim, num_classes = None):
    ...
    def forward(self, x, t, i, lbl = None, time_info = None):
        shape = x.shape
        ...
        if lbl is not None:
            lbl= self.label_emb(lbl)        
            x = self.proj(torch.cat([x, t+lbl, i, time_info], -1))  
        else:
...
def train_diffusion(latent_features, time_info, hidden_dim, num_layers, diffusion_steps, n_epochs, num_classes = None):
    ...
    model = BiRNN_score(input_size, hidden_dim, num_layers, diffusion_steps, time_dim, num_classes).to(device)
...
#####################################################################################################################
@torch.no_grad()
def sample(t, emb, model, time_info, label=None, cfg_scale = 3):
    cov = get_gp_covariance(t)
    L = torch.linalg.cholesky(cov)
    ...
    for diff_step in reversed(range(0, diffusion_steps)):
        ...
        cond_noise = model(x, t, i, label, time_info)
        if cfg_scale > 0:
            uncond_noise = model(x, t, i, None, time_info)
            predicted_noise = torch.lerp(uncond_noise, cond_noise, cfg_scale)
